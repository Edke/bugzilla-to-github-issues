...
namespace {/**
 * Nette Framework (version 2.1-dev released on 2012-05-15, http://nette.org)
 *
...
error_reporting(E_ALL|E_STRICT);@set_magic_quotes_runtime(FALSE);iconv_set_encoding('internal_encoding','UTF-8');extension_loaded('mbstring')&&mb_internal_encoding('UTF-8');umask(0);@header('X-Powered-By: Nette Framework');@header('Content-Type: text/html; charset=utf-8');define('NETTE',TRUE);define('NETTE_DIR',__DIR__);define('NETTE_VERSION_ID',20100);define('NETTE_PACKAGE','5.3');}namespace Nette\Diagnostics{use
Nette;interface
IBarPanel{function
getTab();function
...
setConnection(Connection$connection);}}namespace Nette\Forms{use
Nette;interface
IControl{function
...
new
Nette\InvalidArgumentException('Email address must be a string.');}self::$email=$email;}if(!self::$enabled){register_shutdown_function(array(__CLASS__,'_shutdownHandler'));set_exception_handler(array(__CLASS__,'_exceptionHandler'));set_error_handler(array(__CLASS__,'_errorHandler'));self::$enabled=TRUE;}}static
function
isEnabled(){return
self::$enabled;}static
function
...
new
Responses\RedirectResponse($url,Http\IResponse::S301_MOVED_PERMANENTLY);}}$params=$request->getParameters();if(!isset($params['callback'])){return;}$params['presenter']=$this;$callback=callback($params['callback']);$response=$callback->invokeArgs(Application\UI\PresenterComponentReflection::combineArgs($callback->toReflection(),$params));if(is_string($response)){$response=array($response,array());}if(is_array($response)){if($response[0]instanceof\SplFileInfo){$response=$this->createTemplate('Nette\Templating\FileTemplate')->setParameters($response[1])->setFile($response[0]);}else{$response=$this->createTemplate('Nette\Templating\Template')->setParameters($response[1])->setSource($response[0]);}}if($response
instanceof
...
PresenterFactory
implements
IPresenterFactory{public$caseSensitive=FALSE;public$mapping=array('*'=>'\*Module\*Presenter','Nette'=>'NetteModule\*\*Presenter');private$baseDir;private$cache=array();private$container;function
__construct($baseDir,Nette\DI\Container$container){$this->baseDir=$baseDir;$this->container=$container;}function
createPresenter($name){$presenter=$this->container->createInstance($this->getPresenterClass($name));foreach(array_reverse(get_class_methods($presenter))as$method){if(substr($method,0,6)==='inject'){$this->container->callMethod(array($presenter,$method));}}if($presenter
instanceof
...
formatPresenterClass($presenter){$parts=explode(':',$presenter);$mapping=explode('\\*',isset($parts[1],$this->mapping[$parts[0]])?$this->mapping[array_shift($parts)]:$this->mapping['*']);$class=$mapping[0];while($part=array_shift($parts)){$class.=($class?'\\':'').$part.$mapping[$parts?1:2];}return$class;}function
unformatPresenterClass($class){foreach($this->mapping
as$module=>$mapping){$mapping=explode('\\\\\*',preg_quote($mapping,'#'));$mapping[0].=$mapping[0]?'\\\\':'';if(preg_match("#^\\\\?$mapping[0]((?:\w+$mapping[1]\\\\)*)(\w+)$mapping[2]$#i",$class,$matches)){return($module==='*'?'':$module.':').str_replace($mapping[1].'\\',':',$matches[1]).$matches[2];}}}function
...
setParams(array$params){trigger_error(__METHOD__.'() is deprecated; use setParameters() instead.',E_USER_DEPRECATED);return$this->setParameters($params);}function
getParams(){trigger_error(__METHOD__.'() is deprecated; use getParameters() instead.',E_USER_DEPRECATED);return$this->getParameters();}function
...
getParamId($name){trigger_error(__METHOD__.'() is deprecated; use getParameterId() instead.',E_USER_DEPRECATED);return$this->getParameterId($name);}static
function
...
getWidget($name){trigger_error(__METHOD__.'() is deprecated, use getComponent() instead.',E_USER_DEPRECATED);return$this->getComponent($name);}function
...
addFile($name,$label=NULL){trigger_error(__METHOD__.'() is deprecated; use addUpload() instead.',E_USER_DEPRECATED);return$this->addUpload($name,$label);}}class
Form
extends
Container{const
...
instanceof
ISubmitterControl){if(!$this->submittedBy->getValidationScope()||$this->isValid()){$this->submittedBy->click();$valid=TRUE;}else{$this->submittedBy->onInvalidClick($this->submittedBy);}}if(isset($valid)||$this->isValid()){$this->onSuccess($this);}else{$this->onError($this);if($this->onInvalidSubmit){trigger_error(__CLASS__.'->onInvalidSubmit is deprecated; use onError instead.',E_USER_DEPRECATED);$this->onInvalidSubmit($this);}}if($this->onSuccess){$this->onSubmit($this);}elseif($this->onSubmit){trigger_error(__CLASS__.'->onSubmit changed its behavior; use onSuccess instead.',E_USER_DEPRECATED);if(isset($valid)||$this->isValid()){$this->onSubmit($this);}}}protected
function
...
Responses\RedirectResponse($url,$code));}function
redirectUri($url,$code=NULL){trigger_error(__METHOD__.'() is deprecated; use '.__CLASS__.'::redirectUrl() instead.',E_USER_DEPRECATED);$this->redirectUrl($url,$code);}function
error($message=NULL,$code=Http\IResponse::S404_NOT_FOUND){throw
new
Application\BadRequestException($message,$code);}function
...
function
injectPrimary(Nette\DI\Container$context,Application\Application$application,Http\Context$httpContext,Http\IRequest$httpRequest,Http\IResponse$httpResponse,Http\Session$session,Security\User$user){if($this->application!==NULL){throw
new
...
function
getHttpContext(){return$this->httpContext;}function
getApplication(){return$this->application;}function
...
class
Framework{const
NAME='Nette Framework',VERSION='2.1-dev',REVISION='0f42247 released on 2012-05-15';public
...
Nette\Caching\Storages\FileStorage($cacheDir));$loader->autoRebuild=$this->parameters['debugMode'];return$loader;}function
...
loadConfig($file,$section=NULL){trigger_error(__METHOD__.'() is deprecated; use addConfig(file, [section])->createContainer() instead.',E_USER_DEPRECATED);return$this->addConfig($file,$section)->createContainer();}function
...
function
detectProductionMode($list=NULL){trigger_error(__METHOD__.'() is deprecated; use !detectDebugMode() instead.',E_USER_DEPRECATED);return!static::detectDebugMode($list);}}}namespace Nette\Config\Extensions{use
Nette;use
...
loadConfiguration(){$container=$this->getContainerBuilder();$config=$this->getConfig($this->defaults);$this->setupCache($container);$this->setupHttp($container);$this->setupSession($container,$config['session']);$this->setupSecurity($container,$config['security']);$this->setupApplication($container,$config['application']);$this->setupRouting($container,$config['routing']);$this->setupMailer($container,$config['mailer']);$this->setupForms($container);$this->setupTemplating($container,$config);$this->setupDatabase($container,$config['database']);}private
function
setupCache(ContainerBuilder$container){$container->addDefinition($this->prefix('cacheJournal'))->setClass('Nette\Caching\Storages\FileJournal',array('%tempDir%'));$container->addDefinition('cacheStorage')->setClass('Nette\Caching\Storages\FileStorage',array('%tempDir%/cache'));$container->addDefinition($this->prefix('templateCacheStorage'))->setClass('Nette\Caching\Storages\PhpFileStorage',array('%tempDir%/cache'))->setAutowired(FALSE);$container->addDefinition($this->prefix('cache'))->setClass('Nette\Caching\Cache',array(1=>'%namespace%'))->setParameters(array('namespace'=>NULL));}private
function
setupHttp(ContainerBuilder$container){$container->addDefinition($this->prefix('httpRequestFactory'))->setClass('Nette\Http\RequestFactory')->addSetup('setEncoding',array('UTF-8'))->setInternal(TRUE);$container->addDefinition('httpRequest')->setClass('Nette\Http\Request')->setFactory('@Nette\Http\RequestFactory::createHttpRequest');$container->addDefinition('httpResponse')->setClass('Nette\Http\Response');$container->addDefinition($this->prefix('httpContext'))->setClass('Nette\Http\Context');}private
function
setupSession(ContainerBuilder$container,array$config){$session=$container->addDefinition('session')->setClass('Nette\Http\Session');if(isset($config['expiration'])){$session->addSetup('setExpiration',array($config['expiration']));}if(isset($config['iAmUsingBadHost'])){$session->addSetup('Nette\Framework::$iAmUsingBadHost = ?;',array((bool)$config['iAmUsingBadHost']));}unset($config['expiration'],$config['autoStart'],$config['iAmUsingBadHost']);if(!empty($config)){$session->addSetup('setOptions',array($config));}}private
function
setupSecurity(ContainerBuilder$container,array$config){$container->addDefinition($this->prefix('userStorage'))->setClass('Nette\Http\UserStorage');$user=$container->addDefinition('user')->setClass('Nette\Security\User');if($container->parameters['debugMode']&&$config['debugger']){$user->addSetup('Nette\Diagnostics\Debugger::$bar->addPanel(?)',array(new
...
function
setupTemplating(ContainerBuilder$container,array$config){$latte=$container->addDefinition($this->prefix('latte'))->setClass('Nette\Latte\Engine')->setShared(FALSE);if(empty($config['xhtml'])){$latte->addSetup('$service->getCompiler()->defaultContentType = ?',Nette\Latte\Compiler::CONTENT_HTML);}$container->addDefinition($this->prefix('template'))->setClass('Nette\Templating\FileTemplate')->addSetup('registerFilter',array($latte))->addSetup('registerHelperLoader',array('Nette\Templating\Helpers::loader'))->setShared(FALSE);}private
function
setupDatabase(ContainerBuilder$container,array$config){$container->addDefinition($this->prefix('database'))->setClass('Nette\DI\NestedAccessor',array('@container',$this->prefix('database')));$autowired=TRUE;foreach((array)$config
...
getColumns($table){$columns=array();foreach($this->connection->query("
			...
			ORDER BY
				c.ordinal_position
		")as$row){$row['vendor']=array();$columns[]=(array)$row;}return$columns;}function
...
GroupedSelection($table,$this,$column);}return$referencing->setActive($active)->where("$table.$column",array_keys((array)$this->rows));}function
...
implements
IControl{public
static$idMask='frm-%s';public$caption;protected$value;protected$control;protected$label;private$errors=array();private$disabled=FALSE;private$htmlId;private$htmlName;private$rules;private$translator=TRUE;private$options=array();function
...
NULL;}elseif($this->htmlId===NULL){$this->htmlId=sprintf(self::$idMask,$this->lookup('Nette\Application\IPresenter',FALSE)?$this->lookupPath('Nette\Application\IPresenter'):$this->lookupPath('Nette\Forms\Form'));}return$this->htmlId;}function
...
function
validateValid(IControl$control){return$control->rules->validate(TRUE);}static
function
validateRange(IControl$control,$range){return
Nette\Utils\Validators::isInRange($control->getValue(),$range);}function
...
setPrompt($prompt){$this->prompt=$prompt;if($prompt!==NULL&&!is_bool($prompt)){$this->items=array(''=>$prompt)+$this->items;$this->allowed=array(''=>'')+$this->allowed;}return$this;}function
skipFirst($v=NULL){trigger_error(__METHOD__.'() is deprecated; use setPrompt() instead.',E_USER_DEPRECATED);return$this->setPrompt($v);}final
function
getPrompt(){return$this->prompt;}final
function
areKeysUsed(){return$this->useKeys;}function
setItems(array$items,$useKeys=TRUE){$this->items=$items;if($this->prompt){$this->items=array(''=>is_bool($this->prompt)?$this->items['']:$this->prompt)+$this->items;}$this->allowed=$this->prompt?array(''=>''):array();$this->useKeys=(bool)$useKeys;foreach($items
...
setClientScript(){trigger_error(__METHOD__.'() is deprecated; use unobstructive JavaScript instead.',E_USER_DEPRECATED);return$this;}protected
function
...
getUri(){trigger_error(__METHOD__.'() is deprecated; use '.__CLASS__.'::getUrl() instead.',E_USER_DEPRECATED);return$this->getUrl();}final
function
...
getNamespace($section){trigger_error(__METHOD__.'() is deprecated; use getSection() instead.',E_USER_DEPRECATED);return$this->getSection($section);}function
...
setCookieParams($path,$domain=NULL,$secure=NULL){trigger_error(__METHOD__.'() is deprecated; use setCookieParameters() instead.',E_USER_DEPRECATED);return$this->setCookieParameters($path,$domain,$secure);}function
...
getRelativeUri(){trigger_error(__METHOD__.'() is deprecated; use '.__CLASS__.'::getRelativeUrl() instead.',E_USER_DEPRECATED);return$this->getRelativeUrl();}function
getAbsoluteUri(){trigger_error(__METHOD__.'() is deprecated; use '.__CLASS__.'::getAbsoluteUrl() instead.',E_USER_DEPRECATED);return$this->getAbsoluteUrl();}function
getHostUri(){trigger_error(__METHOD__.'() is deprecated; use '.__CLASS__.'::getHostUrl() instead.',E_USER_DEPRECATED);return$this->getHostUrl();}function
getBaseUri(){trigger_error(__METHOD__.'() is deprecated; use '.__CLASS__.'::getBaseUrl() instead.',E_USER_DEPRECATED);return$this->getBaseUrl();}}class
UrlScript
extends
...
Compiler
extends
Nette\Object{public$defaultContentType=self::CONTENT_XHTML;private$tokens;private$output;private$position;private$macros;private$macroHandlers;private$htmlNode;private$macroNode;private$attrCodes=array();private$contentType;private$context;private$templateId;const
...
CONTEXT_COMMENT='comment',CONTEXT_SINGLE_QUOTED="'",CONTEXT_DOUBLE_QUOTED='"',CONTEXT_UNQUOTED='=';function
...
function
processText(Token$token){if(($this->context[0]===self::CONTEXT_SINGLE_QUOTED||$this->context[0]===self::CONTEXT_DOUBLE_QUOTED)&&$token->text===$this->context[0]){$this->setContext(self::CONTEXT_UNQUOTED);}$this->output.=$token->text;}private
function
processMacroTag(Token$token){$isRightmost=!isset($this->tokens[$this->position+1])||substr($this->tokens[$this->position+1]->text,0,1)==="\n";$this->writeMacro($token->name,$token->value,$token->modifiers,$isRightmost);}private
function
...
HtmlNode($token->name,$this->htmlNode);$this->htmlNode->isEmpty=in_array($this->contentType,array(self::CONTENT_HTML,self::CONTENT_XHTML))&&isset(Nette\Utils\Html::$emptyElements[strtolower($token->name)]);$this->htmlNode->offset=strlen($this->output);$this->setContext(self::CONTEXT_UNQUOTED);}$this->output.=$token->text;}private
function
processHtmlTagEnd(Token$token){if($token->text==='-->'){$this->output.=$token->text;$this->setContext(NULL);return;}$htmlNode=$this->htmlNode;$isEmpty=!$htmlNode->closing&&(Strings::contains($token->text,'/')||$htmlNode->isEmpty);if($isEmpty&&in_array($this->contentType,array(self::CONTENT_HTML,self::CONTENT_XHTML))){$token->text=preg_replace('#^.*>#',$this->contentType===self::CONTENT_XHTML?' />':'>',$token->text);}if(empty($htmlNode->macroAttrs)){$this->output.=$token->text;}else{$code=substr($this->output,$htmlNode->offset).$token->text;$this->output=substr($this->output,0,$htmlNode->offset);$this->writeAttrsMacro($code);if($isEmpty){$htmlNode->closing=TRUE;$this->writeAttrsMacro('');}}if($isEmpty){$htmlNode->closing=TRUE;}if(!$htmlNode->closing&&(strcasecmp($htmlNode->name,'script')===0||strcasecmp($htmlNode->name,'style')===0)){$this->setContext(strcasecmp($htmlNode->name,'style')?self::CONTENT_JS:self::CONTENT_CSS);}else{$this->setContext(NULL);if($htmlNode->closing){$this->htmlNode=$this->htmlNode->parentNode;}}}private
function
processHtmlAttribute(Token$token){if(Strings::startsWith($token->name,Parser::N_PREFIX)){$this->htmlNode->macroAttrs[substr($token->name,strlen(Parser::N_PREFIX))]=$token->value;}else{$this->htmlNode->attrs[$token->name]=TRUE;$this->output.=$token->text;if($token->value){$context=NULL;if(strncasecmp($token->name,'on',2)===0){$context=self::CONTENT_JS;}elseif($token->name==='style'){$context=self::CONTENT_CSS;}$this->setContext($token->value,$context);}}}private
function
...
as$name=>$foo){if(isset($attrs[$name])){if($this->htmlNode->closing){$right[]=array("/$name",'',MacroNode::PREFIX_NONE);}else{array_unshift($left,array($name,$attrs[$name],MacroNode::PREFIX_NONE));}unset($attrs[$name]);}}if($attrs){throw
new
...
as$item){$node=$this->writeMacro($item[0],$item[1],NULL,NULL,$item[2]);if($node->closing||$node->isEmpty){$this->htmlNode->attrCode.=$node->attrCode;if($node->isEmpty){unset($this->htmlNode->macroAttrs[$node->name]);}}}$this->output.=$code;foreach($right
as$item){$node=$this->writeMacro($item[0],$item[1],NULL,NULL,MacroNode::PREFIX_NONE);if($node->closing){$this->htmlNode->attrCode.=$node->attrCode;}}if($right&&substr($this->output,-2)==='?>'){$this->output.="\n";}}function
...
MacroNode($macro,$name,$args,$modifiers,$this->macroNode,$nPrefix===NULL?NULL:$this->htmlNode,$nPrefix);if($macro->nodeOpened($node)!==FALSE){return$node;}}throw
new
...
extends
Nette\Object{const
PREFIX_INNER='inner',PREFIX_TAG='tag',PREFIX_NONE='';public$macro;public$name;public$isEmpty=FALSE;public$args;public$modifiers;public$closing=FALSE;public$tokenizer;public$parentNode;public$openingCode;public$closingCode;public$attrCode;public$content;public$data;public$htmlNode;public$prefix;public$saved;function
__construct(IMacro$macro,$name,$args=NULL,$modifiers=NULL,self$parentNode=NULL,HtmlNode$htmlNode=NULL,$prefix=NULL){$this->macro=$macro;$this->name=(string)$name;$this->modifiers=(string)$modifiers;$this->parentNode=$parentNode;$this->htmlNode=$htmlNode;$this->prefix=$prefix;$this->tokenizer=new
...
finalize(){$last=$this->getCompiler()->getMacroNode();if($last&&($last->name==='block'||$last->name==='#')){$this->getCompiler()->writeMacro('/'.$last->name);}$epilog=$prolog=array();if($this->namedBlocks){foreach($this->namedBlocks
...
macroBlock(MacroNode$node,PhpWriter$writer){$name=$node->tokenizer->fetchWord();if($node->name==='block'&&$name===FALSE){return$node->modifiers===''?'':'ob_start()';}elseif($node->name==='#'){$node->modifiers=substr($node->modifiers,0,-7);}$node->data->name=$name=ltrim($name,'#');if($name==NULL){if($node->name!=='snippet'){throw
new
...
Compiler::CONTEXT_DOUBLE_QUOTED:case
Compiler::CONTEXT_UNQUOTED:if($context[1]===Compiler::CONTENT_JS){$s="Nette\\Templating\\Helpers::escapeJs($s)";}elseif($context[1]===Compiler::CONTENT_CSS){$s="Nette\\Templating\\Helpers::escapeCss($s)";}$quote=$context[0]===Compiler::CONTEXT_SINGLE_QUOTED?', ENT_QUOTES':'';$s="htmlSpecialChars($s$quote)";return$context[0]===Compiler::CONTEXT_UNQUOTED?"'\"' . $s . '\"'":$s;case
...
register($prepend=FALSE){$this->list=$this->getCache()->load($this->getKey(),callback($this,'_rebuildCallback'));parent::register($prepend);return$this;}function
...
setNamespace($namespace){trigger_error(__METHOD__.'() is deprecated; use getStorage()->setNamespace() instead.',E_USER_DEPRECATED);$this->storage->setNamespace($namespace);return$this;}function
getNamespace(){trigger_error(__METHOD__.'() is deprecated; use getStorage()->getNamespace() instead.',E_USER_DEPRECATED);return$this->storage->getNamespace();}function
setAuthenticationHandler($v){trigger_error(__METHOD__.'() is deprecated; use setAuthenticator() instead.',E_USER_DEPRECATED);return$this->setAuthenticator($v);}function
setAuthorizationHandler($v){trigger_error(__METHOD__.'() is deprecated; use setAuthorizator() instead.',E_USER_DEPRECATED);return$this->setAuthorizator($v);}}}namespace Nette\Templating{use
Nette;use
Nette\Caching;class
Template
extends
